...
int main(int argc, char *argv[]) {
  ...
  if (command_line_arguments.count("--time")) {
    ...
    if (command_line_arguments.count("--async-tests") ||
        command_line_arguments.count("--async-tests=1"))
      mode = std::launch::async;
  }
  if (command_line_arguments.count("--sync-tests") ||
      command_line_arguments.count("--async-tests=0"))
    mode = std::launch::deferred;
  ...
  register_test([] {
    ...
    // miscellaneous test of lmj::hash_table ("do random bullsh*t and see if the
    // standard version ends up the same")
  ...
  register_test([] {
    ...
    // test removing elements from lmj::hash_table using std::unordered_map to
    // ensure correctness
    lmj::hash_table<int, int> map;
  ...
  register_test([] {
    ...
    auto hash = [](int x) { return x * x; };
    auto str = [](int x) {
      return "to make it a long string " + std::to_string(x);
    };
